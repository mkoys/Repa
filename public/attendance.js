...
const calendarDatePreviousElement = document.querySelector(
  ".calendarDatePrevious",
);
...
const calendarDatePickerElement = document.querySelector(
  ".calendarDatePickerIcon",
);
const calendarYearDropdownElement = document.querySelector(
...
settingsAction.addEventListener(
  "click",
  (_event) => (window.location = "/settings.html?back=attendance.html"),
);
...
calendarDatePickerElement.addEventListener("click", (event) => {
  if (calendarDatePickerElement.classList.contains("rotate")) {
    ...
    for (let index = currentYear; index >= currentYear - 10; index--)
      allYears.push(index);
    allYears.reverse();
    for (let index = currentYear + 1; index <= currentYear + 10; index++)
      allYears.push(index);
...
attendanceDropdownElement.addEventListener(
  "click",
  () => (window.location.href = "/attendance.html"),
);
usersDropdownElement.addEventListener(
  "click",
...
if (userDataJson.avatar)
  avatarElement.style.backgroudImage = `url(${userDataJson.avatar})`;
...
const attendances = await fetch(
  `/attendance${userIdentifier ? `?id=${userIdentifier}` : ""}`,
  { headers: { Authorization: `token ${token}` } },
...
function calendarEvent(date, action) {
  if (typeof date.multi === "number") {
    const multiElement = document
      .querySelector("#attendanceMultiTemplate")
      .content.cloneNode(true);
    const attendanceDropdownElement = multiElement.querySelector(
      ".attendanceBoxDropdown",
    );
    const multiAttendanceElement =
      multiElement.querySelector(".attendanceMulti");
    const multiContentElement = multiElement.querySelector(
      ".attendanceMultiContent",
    );
    ...
    multiHeader.textContent = `${date.days[0].getDate()}. ${
      MonthsOfYear[date.days[0].getMonth()]
    } ${date.days[0].getFullYear()} - ${date.days[
      date.days.length - 1
    ].getDate()}. ${
      MonthsOfYear[date.days[date.days.length - 1].getMonth()]
    } ${date.days[date.days.length - 1].getFullYear()}`;
    ...
    date.days.forEach((date) => {
      const stateIndex = attendances.findIndex((item) =>
        compareDates(date, item.date),
      );
      if (stateIndex > -1) {
    ...
    date.days.forEach((date) => {
      if (action) {
        const foundAttendanceIndex = attendances.findIndex((attendance) =>
          compareDates(attendance.date, date),
        );
        let attendance;
        if (foundAttendanceIndex > -1) {
          if (date.getDay() != 0 && date.getDay() != 6) {
            ...
            attendance = createAttendance({
              ...
              checkbox: attendanceData.place,
              content: attendanceData.content,
              multi: true,
            });
        ...
        if (attendance) {
          ...
          contentState.push({
            date,
            attendance,
            element: multiAttendanceElement,
          });
          multiClose.addEventListener("click", () => {
            const contentIndex = contentState.findIndex((state) =>
              compareDates(date, state.date),
            );
        ...
        const index = contentState.findIndex((state) =>
          compareDates(date, state.date),
        );
        if (index > -1) {
...
function createAttendance({
  ...
  const attendanceElementCopy =
    attendanceElementTemplate.content.cloneNode(true);
  const attendanceDateElement =
    attendanceElementCopy.querySelector(".attendanceDate");
  const attendanceStatusElement =
    attendanceElementCopy.querySelector(".attendanceStatus");
  const attendanceMoreElement =
    attendanceElementCopy.querySelector(".attendanceMore");
  const attendanceSaveElement =
    attendanceElementCopy.querySelector(".attendanceSave");
  const attendanceSubmitElement =
    attendanceElementCopy.querySelector(".attendanceSubmit");
  const attendanceCheckboxSchool =
    attendanceElementCopy.querySelector("#school");
  const attendanceCheckboxSchoolBox =
    attendanceCheckboxSchool.querySelector(".boxcheck");
  const attendanceCheckboxSchoolCheck =
    attendanceCheckboxSchool.querySelector(".check");
  const attendanceCheckboxCompany =
    attendanceElementCopy.querySelector("#company");
  const attendanceCheckboxCompanyBox =
    attendanceCheckboxCompany.querySelector(".boxcheck");
  const attendanceCheckboxCompanyCheck =
    attendanceCheckboxCompany.querySelector(".check");
  const attendanceCloseElement =
    attendanceElementCopy.querySelector(".attendanceClose");
  const attendanceContentElement =
    attendanceElementCopy.querySelector(".attendanceContent");
  const attendanceAccept =
    attendanceElementCopy.querySelector(".attendanceAccept");
  const attendanceDecline =
    attendanceElementCopy.querySelector(".attendanceDecline");
  const attendanceSaveIcon = attendanceElementCopy.querySelector(
    ".attendanceSaveIcon",
  );
  const attendanceSubmitIcon = attendanceElementCopy.querySelector(
    ".attendanceSubmitIcon",
  );
  const attendanceHeader =
    attendanceElementCopy.querySelector(".attendanceHeader");
  const attendanceDropdownElement = attendanceElementCopy.querySelector(
    ".attendanceBoxDropdown",
  );
  const attendanceDeleteAction =
    attendanceElementCopy.querySelector(".deleteAction");
  const attendanceImportAction =
    attendanceElementCopy.querySelector(".importAction");
  const attendanceFileUpload =
    attendanceElementCopy.querySelector("#attendanceFile");
  const attendanceExportAction =
    attendanceElementCopy.querySelector(".exportAction");
  ...
  attendanceCheckboxCompanyBox.addEventListener("click", (_event) =>
    checkCheckbox(
      attendanceCheckboxCompanyCheck,
      attendanceCheckboxSchoolCheck,
      checkboxDisable,
    ),
  );
  attendanceCheckboxSchoolBox.addEventListener("click", (_event) =>
    checkCheckbox(
      attendanceCheckboxSchoolCheck,
      attendanceCheckboxCompanyCheck,
      checkboxDisable,
  ...
  function setContent(content) {
    ...
    if (content) {
      content.forEach((row) => {
        ...
        const newInputDescription = newInput.querySelector(
        ...
        if (
          ...
          newInput.addEventListener("dragover", (event) =>
          ...
          newInput.addEventListener("dragstart", (event) => {
            ...
            event.dataTransfer.setData(
              "elm",
              Array.from(source.parentNode.children).findIndex(
                (item) => source === item,
              ),
            );
          });
    ...
    const lastInputDescription = lastInput.querySelector(
      ".attendanceInputDescription",
    );
  ...
  async function submitAttendance(event) {
    if (!attendanceContentElement.parentNode.disabled) {
      ...
      Array.from(attendanceContentElement.children).forEach((child) => {
        const inputDescription = child.querySelector(
          ".attendanceInputDescription",
        );
  ...
  function addInputListener(event) {
    if (event.which >= 48) {
      ...
      const inputDescription = parent.querySelector(
        ".attendanceInputDescription",
      );
  ...
  function dragListener(event) {
    ...
    const sourceIndex = Array.from(source.parentNode.children).findIndex(
      (item) => source === item,
    );
  ...
  function removeInputListener(event) {
    if (event.which == 8 || event.which == 46) {
      ...
      const inputTime = parent.querySelector(".attendanceInputTime");
      const inputClass = parent.querySelector(".attendanceInputClass");
      const parentIndex = Array.from(parentOfParent.children).findIndex(
        (child) => child === parent,
      );
      ...
      if (
        ...
        inputTime.value.length == 0 &&
        inputClass.value.length == 0 &&
        parentOfParent.children.length > 1
      ) {
  ...
  function setDate(date) {
    attendanceDateElement.textContent = `${date.getDate()}. ${
  ...
  return {
    ...
    setDate,
    element: attendanceElementCopy,
  };
}

function generateCalendarDates() {
  calendarDatesElement.innerHTML = "";
  calendarDateTextElement.textContent = `${
  ...
  if (currentDays[0].getDay() == 0) {
    while (calendarDays.length < 6) {
      calendarDays.push(
        previousDays[previousDays.length - 1 - calendarDays.length],
      );
    }
  ...
  calendarDays.forEach((day) => {
    ...
    const foundAttendanceIndex = attendances.findIndex((attendance) =>
      compareDates(attendance.date, day),
    );
    if (foundAttendanceIndex > -1) {
    ...
    const foundIndex = calendarState.findIndex((state) =>
      compareDates(state.day, day),
    );
    if (foundIndex > -1) {
    ...
    const rangeIndex = rangeSelect.findIndex((state) =>
      compareDates(state.day, day),
    );
    if (rangeIndex > -1) {
        ...
        if (event.shiftKey) {
          ...
          if (rangeIndex > -1) {
            ...
            if (rangeSelect.length > 1) {
              const big =
                rangeSelect[
                  rangeSelect[0].day.getTime() > rangeSelect[1].day.getTime()
                    ? 0
                    : 1
                ];
              const small =
              ...
              while (!compareDates(current, endDay)) {
                ...
                if (
                  calendarState.findIndex((item) =>
                    compareDates(item.day, current),
                  ) > -1
                ) {
                  while (
                    calendarState.findIndex(
                      (item) => item.multi == rangeCount,
                    ) > -1
                  ) {
                    const index = calendarState.findIndex(
                      (item) => item.multi == rangeCount,
                    );
          ...
          const stateIndex = calendarState.findIndex((state) =>
            compareDates(state.day, day),
          );
          if (stateIndex > -1) {
            if (calendarState[stateIndex].multiselect) {
              ...
              while (
                calendarState.findIndex((item) => item.multi == multi) > -1
              ) {
                const index = calendarState.findIndex(
                  (item) => item.multi == multi,
                );
                calendarState[index].element.classList.remove("rangeSelect");
                calendarState[index].element.classList.remove(
                  "rangeSelectUser",
                );
                calendarState[index].element.classList.remove(
                  "rangeSelectStart",
            ...
            if (!event.ctrlKey) {
              rangeSelect.forEach((item) =>
                item.element.classList.remove("multiSelect"),
              );
              let stop = false;
              while (calendarState.length != 0 && !stop) {
                ...
                calendarEvent(
                  calendarState[calendarState.length - 1].day,
                  false,
                );
                calendarState.pop();
              }
...
function calendarSetDate(date, action) {
  const index = calendarDates.findIndex((dateItem) =>
    compareDates(dateItem, date),
  );
  if (action) {
    if (index > -1)
      calendarDatesElement.children[index].classList.add("calendarDaySelected");
    calendarState.push({
      day: date,
      element: calendarDatesElement.children[index],
    });
  } else {
    if (index > -1) {
      calendarDatesElement.children[index].classList.remove(
        "calendarDaySelected",
      );
