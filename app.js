...
const accessLogStream = fs.createWriteStream(
  path.join(rootFolder, "access.log"),
  { flags: "a" },
);
...
app.get("/user", async (req, res) => {
  ...
  Object.keys(user).forEach((key) => {
    if (removeKeys.findIndex((removeKey) => removeKey === key) > -1)
      delete user[key];
  });
...
app.get("/users", async (req, res) => {
  const token = req.headers.authorization.split(" ")[1];
  if (!token)
    return res.json({ error: { id: 5, message: "User not authorized" } });
  const session = await sessions.findOne({ token });
  if (!session)
    return res.json({ error: { id: 6, message: "User not authorized" } });
  const userRequest = await users.findOne({
    id: session.id,
    deleted: { $exists: false },
  });
  if (userRequest.role !== "admin")
    return res.json({ error: { id: 7, message: "User not authorized" } });
  ...
  if (userRequest.autoFilter)
    Object.keys(userRequest.autoFilter).forEach(
      (key) => (filterParam[key] = userRequest.autoFilter[key]),
    );
  ...
  const page = await users
    .find({ $or: [filterParam, { id: userRequest.id }] })
    .skip((pageNumber > 0 ? pageNumber : 0) * pageVisible)
    .limit(pageVisible)
    .sort(sort ? sortParam : { id: 1 })
    .toArray();

  const pageLength =
    Math.floor(userLength / pageVisible) +
    (userLength % pageVisible != 0 ? 1 : 0);
...
app.delete("/user", async (req, res) => {
  ...
  await users.updateOne(
    { id: data.id },
    { $set: { deleted: { by: session.id, stamp: new Date() } } },
  );
  res.json({ message: "ok" });
});
...
app.post("/user", async (req, res) => {
  ...
  if (
    ...
    return res.json({ error: { id: 2, message: "Empty values" } });

  const user = await users.findOne({
    $or: [
      { username: data.username, deleted: { $exists: false } },
      { email: data.email, deleted: { $exists: false } },
    ],
  });
  if (user)
    return res.json({ error: { id: 3, message: "User already exists" } });
...
app.post("/register", async (req, res) => {
  ...
  const user = await users.findOne({
    $or: [
      {
        username: {
          $regex: new RegExp("^" + data.username.toLowerCase() + "$", "i"),
        },
        deleted: { $exists: false },
      },
      { email: data.email.toLowerCase(), deleted: { $exists: false } },
    ],
  });
  if (user)
    return res.json({ error: { id: 3, message: "User already exists" } });
...
app.post("/login", async (req, res) => {
  const data = req.body;
  if (data.password === undefined || (!data.username && !data.email))
    return res.json({ error: { id: 0, message: "Invalid request" } });
  if (
    (typeof data.email !== "string" && typeof data.username !== "string") ||
    typeof data.password !== "string"
  )
    return res.json({ error: { id: 1, message: "Invalid type" } });
  if (
  ...
  if (!user)
    return res.json({ error: { id: 4, message: "Invalid credentials" } });
...
app.get("/attendance", async (req, res) => {
  ...
  if (!session)
    return res.json({ error: { id: 5, message: "User not authorized" } });
  if (req.query.id) {
    const attendancesResult = await attendances
      .find({ user: req.query.id })
      .toArray();
    if (!attendancesResult)
      return res.json({ error: { id: 7, message: "User not found" } });
...
app.put("/attendance", async (req, res) => {
  ...
  if (req.query.id) {
    updateAttendance = await attendances.updateOne(
      { user: req.query.id, date },
      { $set: { status: req.query.status ? parseInt(req.query.status) : 2 } },
    );
  } else {
    updateAttendance = await attendances.updateOne(
      { user: session.id, date },
      { $set: { status: req.query.status ? parseInt(req.query.status) : 2 } },
    );
  }

  if (updateAttendance.modifiedCount == 0)
    return res.json({ error: { id: 10, message: "No saved attendance" } });
  res.json({ message: "ok" });
});
...
app.post("/attendance", async (req, res) => {
  ...
  if (req.query.id) {
    foundAttendance = await attendances.findOne({
      user: req.query.id,
